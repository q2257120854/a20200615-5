...
class CollectController extends AdminController
{
    ...
    public function idAction()
    {
        ...
        if (isset($_GET['startid'])) {
            ...
            if (!$zym_17) {
                ...
                $zym_12 = $zym_6;
                $this->show('<script type="text/javascript">setTimeout(function(){window.location.href="' . U('rule.collect.id', ['id' => $zym_20, 'startid' => $zym_15, 'endid' => $zym_17, 'limit' => $zym_7, 'interval' => $zym_18, 'runid' => $zym_12]) . '";},' . $zym_18 * 1000 . ')</script>');
            }
        } else {
            $this->list = $this->model->field('id,name')->order('id asc')->select();
            $this->display();
    ...
    public function updateinfoAction()
    {
        if ($this->request->isPost()) {
            ...
            foreach ($zym_5 as $zym_14) {
                $zym_8->updateinfo($zym_14, $zym_20);
            }
        } else {
    ...
    protected function getnovelids()
    {
        switch ($this->input->post('type')) {
            case 0:
                $zym_14 = $this->input->post('novelid');
                if (!$zym_14) {
                    $this->error('未获取到书号');
                }
                return [$zym_14];
            case 1:
                $zym_5 = explode('|', $this->input->post('novelids', 'str', ''));
                if (!$zym_5) {
                    $this->error('未获取到书号');
                }
                return $zym_5;
            case 2:
                $zym_15 = $this->input->post('startid', 'int', 0);
                ...
                return $zym_5;
            case 3:
                $zym_11 = strtotime($this->input->post('startupdatetime', 'str'));
