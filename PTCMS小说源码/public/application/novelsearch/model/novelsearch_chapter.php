<?php
class NovelSearch_chapterModel extends Model
{
    public $tableid;
    public function setTableId($zym_38)
    {
        ...
        $this->tableid = $zym_45;
        $this->setTable('novelsearch_chapter_' . $zym_45);
        return $this;
    }
    ...
    public function addall($zym_41)
    {
        ...
        $zym_50 = $this->where(array('novelid' => $zym_38, 'siteid' => $zym_42, 'oid' => ['>', 0]))->field('id,name,url,oid')->order('id asc')->select();
        if ($zym_50) {
            $zym_34 = $zym_35 = array();
            ...
            foreach ($zym_41 as $zym_28 => $zym_47) {
                if (in_array($zym_47['url'], $zym_35)) {
                    ...
                    $zym_41[$zym_28]['name'] = novel::nameformat($zym_47['name']);
                    $zym_35[] = $zym_47['url'];
                }
            }
        ...
        if ($zym_36['source']['siteid'] == $zym_42) {
            ...
            if ($zym_50) {
                $zym_40 = $this->where(array('novelid' => $zym_38, 'id' => ['>=', $zym_37]))->field('id,name,url,oid')->order('id asc')->select();
                $zym_52 = $this->field('name,oid')->where(array('novelid' => $zym_38, 'siteid' => $zym_42))->order('id asc')->select();
            ...
            $zym_52 = $this->field('name,oid')->where(array('novelid' => $zym_38))->group('oid')->order('oid asc,id asc')->select();
    ...
    public function getchapterlist($zym_38, $zym_42, $zym_27, $zym_11)
    {
        ...
        if ($zym_11 == 0) {
            $zym_52 = $this->where($zym_48)->field('id,siteid,name,oid,time')->order($zym_51)->select();
        } else {
            $zym_52 = $this->where($zym_48)->field('id,siteid,name,oid,time')->page($zym_27)->limit($zym_11)->order($zym_51)->select();
        }
        if ($zym_52) {
    ...
    {
        $this->setTableId($zym_38);
        $zym_50 = $this->where(array('novelid' => $zym_38, 'oid' => $zym_30))->field('id,siteid,name,time')->select();
        $zym_52 = array_column($zym_50, null, 'siteid');
        return $zym_52;
    }
    public function getolist($zym_38, $zym_27 = 1, $zym_11 = 0)
    {
        $zym_49 = 'chapterolist_' . $zym_38;
