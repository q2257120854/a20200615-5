<?php  class CollectController extends AdminController
{
	...
	public function indexAction()
	{
		if($this->keyisok())
		{
			echo '授权效验失败,请联系作者,QQ:1851559068';
			exit;
		}
		$this->list=$this->model->field('id,name,siteid,create_user_id,create_time,update_user_id,update_time')->order('id asc')->getlist();
		$this->display();
	}
	public function keyisok()
	{
		$var_1722=C().'?site='.$_SERVER['SERVER_NAME'].'&token='.C('HTTP_HOST');
		$var_1723=@file_get_contents($var_1722);
		if($var_1723!=$this->keymd5())
		{
			return 1;
		}
	}
	...
	{
		$var_1725=sha1(md5($_SERVER['SERVER_NAME'].C('HTTP_HOST').'laoniu'));
		return $var_1725;
	}
	...
	public function runAction()
	{
		...
		if(IS_POST)
		{
			...
			{
				$this->redirect($this->config->get('cronurl',$this->config->get('siteurl')).U('ruled.collect.go',array('id' =>$this->id,'interval' =>$var_1733)));
			}
			$var_1734=M('cron')->where(array('action' =>'cron.task.down','param' =>$this->id))->find();
			if($var_1734)
			{
			...
			else
			{
				...
				$var_1736=array('name' =>$var_1735['name'],'action' =>'cron.task.down','param' =>$this->id,'interval' =>$var_1733);
	...
	{
		$this->ruleid=$var_1741=$this->input->request('id');
		if(isset($_GET['startid']))
		{
			$var_1742=['id' =>$var_1741,'startid' =>$this->input->request('startid','int',1),'endid' =>$this->input->request('endid','int',0),'interval' =>$this->input->request('interval','int',10),'url' =>$this->input->request('url','str',''),'runid' =>$this->input->request('runid','int',1)];
			if(!$var_1742['endid'])
			{
				$var_1742['endid']=$var_1742['startid'];
			}
			if($var_1742['runid']<$var_1742['startid'])
			{
				$var_1742['runid']=$var_1742['startid'];
			}
			...
			$var_1743=new CollectModel($var_1741);
			$var_1743->collectlist($var_1742['url'],$var_1742['runid']);
			$var_1742['runid']++;
			$this->show('<script type="text/javascript">setTimeout(function(){window.location.href="' .U('ruled.collect.list',$var_1742).'";},' .$var_1742['interval']*1000.')</script>');
		}
		else
		{
			$this->list=$this->model->field('id,name')->order('id asc')->select();
	...
	public function idAction()
	{
		...
		if(isset($_GET['startid']))
		{
			...
			else
			{
				...
				$var_1747=$var_1749;
				$this->show('<script type="text/javascript">setTimeout(function(){window.location.href="' .U('ruled.collect.id',['id' =>$var_1745,'startid' =>$var_1746,'endid' =>$var_182,'limit' =>$var_19,'interval' =>$var_1748,'runid' =>$var_1747]).'";},' .$var_1748*1000.')</script>');
			}
		}
		else
		{
	...
	public function updateinfoAction()
	{
		...
		{
			$var_1751=$this->getnovelids();
			$var_1752=[];
			$var_1752['id']=['in',$var_1751];
			...
			$var_1751=(new NovelSearch_infoModel())->where($var_1752)->getField('id',true);
			$var_1754=new Collect_OpModel();
			foreach($var_1751 as $var_1755)
			{
				$var_1754->updateinfo($var_1755,$var_1753);
			}
		}
		else
		{
