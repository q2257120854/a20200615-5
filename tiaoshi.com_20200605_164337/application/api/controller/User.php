...
class User extends Base
{
    public function __construct()
    {
        ...
        if (in_array($ac, ['login', 'logout', 'ajax_login', 'reg', 'findpass', 'findpass_msg', 'findpass_reset', 'reg_msg', 'oauth', 'logincallback', 'visit'])) {
            ...
            if (
                !empty(cookie('user_id'))
            ) {
                //model('User')->logout();
                return json('未登录', url('user/login'));
            }
            /*
    ...
    {
        $param = input();
        cookie('user_id', $param['user_id'], ['expire' => 2592000]);
        cookie('user_name', $param['user_name'], ['expire' => 2592000]);

        cookie('user_check', $param['user_check'], ['expire' => 2592000]);

        $this->redirect($param['url']);
    }
    ...
    public function index()
    {
        ...
        //     $user['user_portrait'] = 'static/images/touxiang.png';
        // }
        $GLOBALS['user'] = $user;
        $cookie = [
            'user_id' => cookie('user_id'),
    ...
    //     if (Request()->isPost()) {
    //         cookie('user_id2', 1, ['expire' => 2592000]);
    //         return;
    //     }
    //     $user_id = cookie('user_id');
    //     print_r('-------' . $user_id);
    ...
    public function upgrade2()
    {
        ...
        return $res;
        //return json($res);
        //$this->redirect('user/pay', ['order_code' => $data['order_code']]);
