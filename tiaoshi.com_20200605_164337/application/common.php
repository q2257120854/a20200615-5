...
function M3u8PrivateKeyA($url){
  $data = parse_url($url);//切割url地址
  $time=strtotime("+1 hours");//失效时间为1个小时后
  $key="123456789";//加密密钥，替换为你的密钥
  $domain=$data['scheme'].'://'.$data['host'];//拼接出域名
  $filename=$data['path'];//文件访问路径
  $sstring = $filename."-".$time."-0-0-".$key;//拼凑字符串
  $md5=md5($sstring);//MD5方式加密字符串
