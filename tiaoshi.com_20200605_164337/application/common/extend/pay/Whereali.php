...
class Whereali {
    ...
    public function submit($user,$order,$param)
    {
		...
		$data = array(
          ...
          // "notify_url" => $GLOBALS['http_type'] . $_SERVER['HTTP_HOST'] . '/index.php/payment/notify/pay_type/wpay',//通知地址
           //"return_url" => $GLOBALS['http_type'] . $_SERVER['HTTP_HOST'] . '/index.php/payment/notify/pay_type/wpay',//跳转地址
		);
		...
		$query = implode('&',$urls);
        $url = "/demo/Wali/epayapi.php?{$query}"; //支付页面
    ...
    public function notify()
    {
		...
		$res = model('Order')->notify($param['out_trade_no'],'Whereali');
		if($res['code'] >1){
			echo 'fail2';
		}
		else {
