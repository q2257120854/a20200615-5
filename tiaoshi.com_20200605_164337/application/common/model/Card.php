...
class Card extends Base {
    ...
    public function saveAllData($num,$money,$point,$role_no,$role_pwd,$vip,$tian)
    {
        ...
        $t = time();
        for($i=1;$i<=$num;$i++){
            $card_no = $money.'AAA'.mac_get_rndstr(16,$role_no);
    ...
    public function useData($card_no,$user_info)
    {
		...
		if($info['card_vip']!==""){
			...
			Db::query("UPDATE  `facai_user` SET  `group_id` =  '".$info['card_vip']."',`user_end_time` =  '".$time."' WHERE  `facai_user`.`user_id` ='".$user_info['user_id']."';");
		}else{
			$where2=[];
