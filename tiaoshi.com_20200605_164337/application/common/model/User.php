...
class User extends Base
{
    ...
    public function saveData($data)
    {
        ...
        if (!empty($data['user_id'])) {
            ...
            if (empty($data['code'])) {
				...
				$where = [];
				$where['user_id'] = ['neq', $data['user_id']];
				$where['code'] = ['eq', $data['code']];
				$re = $this->where($where)->find();	
				if($re){
					return ['code' => 1001, 'msg' => '参数错误：code已被占用' ];
				}
			}
			$where = [];
    ...
    public function login($param)
    {
        ...
        if(empty($row)) {
            return ['code' => 1003, 'msg' => '您的账号或密码输入有误,请检查'];
        }
    ...
    public function upgrade($param)
    {
        ...
        $long = $param['long'];
        $points_long = ['day'=>86400,'week'=>86400*7,'month'=>86400*30,'year'=>86400*365,'along'=>9999];
