...
class User extends Base
{
	...
	public function foo_input(){
		$rs = input();
       
		if($rs['code']){
			$w['code'] = $rs['code'];
			$rr = Db::name('User')->where($w)->find();
			if($rr['user_id']){
				$rs['uid'] = $rr['user_id'];
			}

		}elseif(cookie('foo_id')){
			$rs['uid'] = cookie('foo_id');
		}elseif(cookie('tg_code')){
         
			////20190925新增tg_code,查询注册码对应的用户id
			$w['code'] = cookie('tg_code');
			$rr = Db::name('User')->where($w)->find();
    ...
    public function reg()
    {
        ...
        if (Request()->isPost()) {
          	//20190925新增tg_code
         	if (!empty(cookie('tg_code'))) {
                $param['uid'] = intval( $param['uid']);
            }else if (!empty(cookie('uid'))) {
  ...
  public function gopay2()
    {
        ...
        */
        //创建订单
        		$price = strtolower(htmlspecialchars(urldecode(trim($param['price']))));
                //$price = $group_info['group_points_'.$long];
                if (empty($price)) {
                    return json(['code' => 1001, 'msg' => '参数错误']);
                }
                ...
                $data['order_price'] = $price;
                $data['order_time'] = time();
                $data['order_points'] = 0;//$group_id;
                //$data['order_remarks'] = $long;
                
                $res = model('Order')->saveData($data);
    			//print_r($res);
    	
        $order_code = htmlspecialchars(urldecode(trim($data['order_code'])));
        ...
        $res = model('Order')->infoData($where);
    */
    	$order_id = intval((trim($res['info']['order_id'])));
    ...
    {
    //  $where2=[];
    //    $where2['user_id'] = 11138;
    //    $user = model('User')->infoData($where2);
    //  	 print_r($user['info']);
      model('order')->notify('PAY20200515025452739451','juhe');
   }
