...
if(isset($_POST['clearvpn'])){
	$result=mysql_query("UPDATE chzb_users set vpn=0");
    echo "抓包记录已清空";
}
if(isset($_POST['stopuse'])){
	$name=$_POST['name'];
    $now=time();
	$result=mysql_query("UPDATE chzb_users set status=0 where name=$name and status>0");
	$result=mysql_query("UPDATE chzb_users set exp=$now where name=$name and status<0");
}
if(isset($_POST['startuse'])){
	$name=$_POST['name'];
	$result=mysql_query("UPDATE chzb_users set status=1 where name=$name and status=0");
...
if(isset($_POST['submitclearold'])){
	...
	mysql_query("delete from chzb_loginrec where logintime<$oldtime");
}
if(isset($_POST['submitclearall'])){
	mysql_query("delete from chzb_loginrec");
}
//获取每日允许登陆IP数量
...
<div id="loginrec">
	<table  border="1" bordercolor="#a0c6e5" style="border-collapse:collapse;">
		...
		<tr><td width="100px">账号</td>
			<td width="200px">登陆信息</td>
			<td width="200px">登陆信息</td>
			<td width="200px">登陆信息</td>
			<td width="200px">登陆信息</td>
			<td width="200px">登陆信息</td>
			<td width="80px">IP数量</td></tr>

	<?php
		$pre24time=strtotime(date("Y-m-d"),time());
		$result=mysql_query("SELECT userid,deviceid,mac,model,ip,region,logintime from chzb_loginrec where logintime>$pre24time order by userid,deviceid,mac,model");
	...
	?>
     ...
     <table border="1" bordercolor="#a0c6e5" style="border-collapse:collapse;">
      ...
      <tr><td  width="100px">账号</td>
        ...
        <td  width="300px" colspan="2">操作&nbsp;&nbsp;&nbsp;&nbsp;<form method="POST"><input type='submit' name='clearvpn' value='清空抓包数据'></form>
        </td>
			
	  </tr>
			<?php
			$result=mysql_query("SELECT status,name,model,vpn,marks,exp from chzb_users where vpn>0");
			while ($row=mysql_fetch_array($result)) {
				$vpn=$row['vpn'];
				$name=$row['name'];
				$marks=$row['marks'];
