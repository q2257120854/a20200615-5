...
mysql_query("UPDATE chzb_proxy set src='$src1',proxy='$proxy1' where id>0 limit 1");
...
if(isset($_POST['submit'])&&isset($_POST['newpassword'])){
	...
	$result=mysql_query("select * from chzb_admin where name='$username' and psw='$oldpassword'");
...
if(isset($_POST['adminadd'])){
	...
	$result=mysql_query("SELECT count(*) from chzb_admin");
	if($row=mysql_fetch_array($result)){
		if($row[0]>5){
			echo"<script>showindex=5;alert('管理员数量已达上限！');</script>";	
		}else{
			$result=mysql_query("select * from chzb_admin where name='$adminname'");
			if(mysql_fetch_array($result)){
				echo"<script>showindex=6;alert('用户名已存在！');</script>";	
			}else{
				mysql_query("INSERT into chzb_admin (name,psw) values ('$adminname','$adminpsw')");
...
if(isset($_POST['deleteadmin'])){	
	...
	foreach ($_POST['adminname'] as $name) {
		if($name<>'admin'){
			mysql_query("delete from chzb_admin where name='$name'");
...
if(isset($_POST['submitsaveproxy'])){
	echo"<script>showindex=3;</script>";

	mysql_query("delete from chzb_proxy");	
	for($i=0;$i<count($_POST['src']);$i++){
		$src1=$_POST['src'][$i];
		$proxy1=$_POST['proxy'][$i];
		if($src1!=''&&$proxy1!=''){
			$proxy1Encoded=base64_encode(gzcompress($proxy1));
			$result=mysql_query("SELECT * from chzb_proxy where proxy='$proxy1Encoded'");
			if(mysql_num_rows($result)>0){
				echo"<script>showindex=3;alert('请勿设置相同的播放格式！[$proxy1]');</script>";
			}else{
				$src1=base64_encode(gzcompress($src1));
				$proxy1=base64_encode(gzcompress($proxy1));
				mysql_query("INSERT into chzb_proxy (src,proxy) values ('$src1','$proxy1')");
			}
...
if(isset($_POST['saveauthorinfo'])&&isset($_POST['author1'])){
	...
	mysql_query("UPDATE chzb_admin set author1=0,author2=0,useradmin=0,channeladmin=0,ipcheck=0 where name<>'admin'");
	foreach ($_POST['author1'] as $adminname){		
		mysql_query("UPDATE chzb_admin set author1=1 where name='$adminname'");			
	}
	foreach ($_POST['author2'] as $adminname){
		mysql_query("UPDATE chzb_admin set author2=1 where name='$adminname'");		
	}
	foreach ($_POST['useradmin'] as $adminname){
		mysql_query("UPDATE chzb_admin set useradmin=1 where name='$adminname'");		
	}
	foreach ($_POST['channeladmin'] as $adminname){
		mysql_query("UPDATE chzb_admin set channeladmin=1 where name='$adminname'");	
	}
	foreach ($_POST['ipcheck'] as $adminname){
		mysql_query("UPDATE chzb_admin set ipcheck=1 where name='$adminname'");	
	}
...
if(isset($_POST['submittipset'])){
	...
	$tipmatcherror=$_POST['tipmatcherror'];
	mysql_query("update chzb_appdata set tiploading='$tiploading',tipusernoreg='$tipusernoreg',tipuserexpired='$tipuserexpired',tipuserforbidden='$tipuserforbidden',tipmatcherror='$tipmatcherror'");
...
if(isset($_POST['submitback'])){
	$result=mysql_query("select name,deviceid,mac,model,author,exp,marks,status from chzb_users where status>-1");
...
if(isset($_POST['submitrestore'])){
	...
	foreach($lines as $line){	
		if (strpos($line, ',') !== false){
			...
			if(mysql_fetch_array($result)){
				$failedCount++;
				echo "$line  因ID已存在导入失败<br>";
			}else{
				if(mysql_query("INSERT into chzb_users (name,mac,deviceid,model,author,exp,status,marks) values($name,'$mac','$deviceid','$model','$author',$exp,$status,'$marks')")){
					$sucessCount++;
				}else{
					$failedCount++;
				}				
			...
			echo "$line  因格式错误导入失败<br>";
			$failedCount++;
		}
	}
	echo "导入完成。成功$sucessCount 条,失败$failedCount 条。";
	echo"<script>showindex=1;</script>";
}

if(isset($_POST['submitrestoreid'])){
	$userdata=$_POST['userdata'];
	$days=$_POST['exp'];
	$marks=$_POST['marks'];
	$lines=explode("\r\n",$userdata);
	...
	foreach($lines as $line){	
			...
			$exp=$days;
			$name=$line;
			$result=mysql_query("select sn from chzb_serialnum where sn=$name");
			if(mysql_fetch_array($result)){
				$failedCount++;
				$failedname.="[$name]";
			}else{
				mysql_query("INSERT into chzb_serialnum (sn,exp,author,createtime,marks) values ($name,$exp,'$user',$nowtime,'$marks')");
				$sucessCount++;
			}
			
		}
	}
	echo "导入完成。成功$sucessCount 条,失败$failedCount 条。$failedname 导入失败，原因：系统已存在此账号。";
	echo"<script>showindex=1;</script>";
}
...
if(isset($_POST['submitsplash'])){
	if ($_FILES["splashbj"]["type"] == "image/png"){
		if ($_FILES["splashbj"]["error"] > 0){
			...
			$savefile="bj/".$_FILES["splashbj"]["name"];
			...
			mysql_query($sql);
			echo "上传成功，图片地址：<a target='_blank' href='$bjurl'>预览</a>";
		}
	}else{
		echo "图片仅支持PNG格式，不超过800KB，多张图片为随机显示。";
	}
	echo"<script>showindex=4;</script>";
}
if(isset($_POST['submitdelbg'])){
	$file=$_POST['file'];
	unlink('bj/'.$file);
	echo"<script>showindex=4;alert('删除成功！');</script>";
}
...
if(isset($_POST['submitcloseauthor'])){
	...
	mysql_query("UPDATE chzb_appdata set needauthor=$needauthor");
}
...
$result=mysql_query("select dataver,appver,setver,dataurl,appurl,adtext,showtime,showinterval,splashbj,exitbj,needauthor,decoder,buffTimeOut,tiploading,tipuserforbidden,tipuserexpired,tipusernoreg,tipmatcherror,trialdays,qqinfo from chzb_appdata");
if($row=mysql_fetch_array($result)){
	$adtext=$row['adtext'];
	$dataver=$row['dataver'];
	$appver=$row['appver'];
	...
	$tipmatcherror=$row['tipmatcherror'];
...
<div  class='blogbox' ><br>
	...
	<li><span align="left"><div class="title">系统备份</div></span>
		...
		<input type="submit" name="submitrestoreid" value="用户账号导入">

		<input type="submit" name="submitback" value="导出用户数据">
	...
	<li id="bg"><span align="left"><div class="title">背景图片</div></span>		
		...
		<br><font color="red">提示：图片仅支持PNG格式，不超过800KB，多张图片为随机显示。</font>
	</li>
	...
	<li><span align="left"><div class="title">修改密码</div></span>
		<br><br>
		<form method="post" align=center>
		用户名:<input type="text" name="username" value="" size="80"><br>
	...
	<li><span align="left"><div class="title">管理员设定</div></span>
		...
		<table border="1" bordercolor="#00f" style="border-collapse:collapse;margin:20px">
			...
			<?php
			$result=mysql_query("select name,author1,author2,useradmin,channeladmin,ipcheck from chzb_admin");
			while ($row=mysql_fetch_array($result)) {
				$adminname=$row['name'];
				$author1=$row['author1'];
				$author2=$row['author2'];
