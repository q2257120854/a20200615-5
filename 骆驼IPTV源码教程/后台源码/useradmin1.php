...
mysql_query("alter table chzb_serialnum add column vip int DEFAULT 0;");
function genName(){
...
if(isset($_POST['submitclear'])){
		mysql_query("delete from chzb_serialnum where enable=1");		
	echo "已清空激活授权号！";
}
...
if(isset($_POST['submitadddays'])){
	$days=$_POST['exp'];
	foreach ($_POST['sn'] as $sn){
		mysql_query("UPDATE chzb_serialnum set exp=$days where sn=$sn and enable=0");		
	}		
	echo "选中用户天数已修改！";
}
if(isset($_POST['submitmodifymarks'])){
	$marks=$_POST['marks'];
	foreach ($_POST['sn'] as $sn){
		mysql_query("UPDATE chzb_serialnum set marks='$marks' where sn=$sn and enable=0");		
	}
	echo "选中用户备注已修改！";
}
...
$result=mysql_query("select count(*) from chzb_serialnum");
if($row=mysql_fetch_array($result)){
	$userCount=$row[0];
