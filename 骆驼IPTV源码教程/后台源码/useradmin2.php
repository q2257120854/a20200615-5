...
mysql_query("alter table chzb_users add column vip int DEFAULT 0;");
...
if(isset($_POST['submitdel'])){
	foreach ($_POST['id'] as $id){
		mysql_query("delete from chzb_users where name=$id");	
		mysql_query("delete from chzb_loginrec where userid=$id");	
	}
	echo "选中用户及其登陆信息已删除！";
}
if(isset($_POST['submitsetvip'])){
	foreach ($_POST['id'] as $id){
		mysql_query("update chzb_users set vip=1 where name=$id");	
	}
	echo "选中用户已开启隐藏节目！";
}	
if(isset($_POST['submitclearvip'])){
	foreach ($_POST['id'] as $id){
		mysql_query("update chzb_users set vip=0 where name=$id");	
	}
	echo "选中用户已关闭隐藏节目！";
}	
if(isset($_POST['submitmodify'])){
	...
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set exp=$exp where name=$id and status=1");
		mysql_query("update chzb_users set exp=$expimportmac where name=$id and status=2");		
	}
	echo "选中用户授权天数已修改！";	
}
if(isset($_POST['submitadddays'])){
	...
	$exp=86400*$_POST['exp'];
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set exp=exp+$exp where name=$id and status=1");	
		mysql_query("update chzb_users set exp=exp+$expimportmac where name=$id and status=2");	
	}
	echo "选中用户授权天数已增加！";	
}
if(isset($_POST['submitmodifymarks'])){
	$marks=$_POST['marks'];
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set marks='$marks' where name=$id");		
	}
	echo "选中用户备注已修改！";	
}
...
if(isset($_POST['submitforbidden'])){
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set status=0 where name=$id and (status=1 or status=999)");		
	}
	echo "选中用户已停用！";	
}
...
if(isset($_POST['submitNotExpired'])){
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set status=999 where name=$id and status=1");		
	}
	echo "选中用户已设置为永不到期！，未激活用户无法设置！";	
}
if(isset($_POST['submitCancelNotExpired'])){
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set status=1 where name=$id and status=999");		
	}
	echo "选中用户已取消永不到期权限！";	
}
if(isset($_POST['submitpermit'])){
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set status=1 where name=$id and (status=0 or status=999)");		
	}
	echo "选中用户已启用！";	
}
...
if(isset($_POST['submitclear'])){
	foreach ($_POST['id'] as $id) {
		mysql_query("update chzb_users set mac='',deviceid='',model='' where name=$id");
		mysql_query("delete chzb_loginrec from where userid=$id");
	}	
	echo "选中用户绑定信息已清除！";
}
if(isset($_POST['submitmodifyipcount'])){
	$ipcount=$_POST['ipcount'];
	mysql_query("update chzb_appdata set ipcount=$ipcount");
}

if(isset($_POST['recCounts'])){
	$recCounts=$_POST['recCounts'];
	mysql_query("update chzb_admin set showcounts=$recCounts where name='$user'");
}
...
//获取每页显示数量
$result=mysql_query("select showcounts from chzb_admin where name='$user'");
...
//获取每日允许登陆IP数量
$result=mysql_query("select ipcount from chzb_appdata");
if($row=mysql_fetch_array($result)){
	$ipcount=$row['ipcount'];
}else{
	$ipcount=5;
...
//获取用户总数并根据每页显示数量计算页数
$result=mysql_query("select count(*) from chzb_users where status>-1");
if($row=mysql_fetch_array($result)){
	$userCount=$row[0];
...
$result=mysql_query("select count(*) from chzb_users where status>-1 and lasttime>$todayTime");
if($row=mysql_fetch_array($result)){
	$todayuserCount=$row[0];
}else{
	$todayuserCount=0;
}
$result=mysql_query("select count(*) from chzb_users where status>-1 and authortime>$todayTime");
if($row=mysql_fetch_array($result)){
...
$nowTime=time();
$result=mysql_query("select count(*) from chzb_users where status=1 and exp<$nowTime");
if($row=mysql_fetch_array($result)){
	$expuserCount=$row[0];
}else{
	$expuserCount=0;
...
<table border="1" bordercolor="#a0c6e5" style="border-collapse:collapse;">
	...
	<tr><td width=70><a href="?order=name">账号<?php if($order=='name')echo'↓';?></a></td>
		<td width=140><a href="?order=mac">MAC<?php if($order=='mac')echo'↓';?></a></td>
		<td width=140><a href="?order=deviceid">AndroidID<?php if($order=='deviceid')echo'↓';?></a></td>
		<td width=140><a href="?order=model">Model<?php if($order=='model')echo'↓';?></a></td>
...
<tr><td colspan="10">
	...
	<input type="submit" name="submitsetvip" value="开启隐藏节目">&nbsp;&nbsp;&nbsp;
	<input type="submit" name="submitclearvip" value="关闭隐藏节目">&nbsp;&nbsp;&nbsp;

	<br><br>
	<input type="text" name="marks" value="" size="15">
